...
{
	public:
		Solver_MCSVM_CS(const problem *prob, int nr_class, double *C, double *C_vec, double eps=0.1, int max_iter=100000);
		~Solver_MCSVM_CS();
		void Solve(double *w);
	private:
...
Solver_MCSVM_CS::Solver_MCSVM_CS(const problem *prob, int nr_class, double *weighted_C, double *C_vec, double eps, int max_iter)
{
	this->w_size = prob->n;
	this->l = prob->l;
...
extern "C" model* train(const problem *prob, const parameter *param)
{
	...
	else
	{
		...
		if(param->solver_type == MCSVM_CS)
		{
			...
			Solver_MCSVM_CS Solver(&sub_prob, nr_class, weighted_C, param->W_bias, param->eps);
